...
import sys
import datetime
from file2dw.tasks import run_task
...
sql = "select distinct a.* from (select * from file2dw_log where game='%s' and platform='%s' and load_status=0 " \
      "and do_rate='%s' and task_date='%s' and in_queue=0 and exec_num<4 and log_time<='%s') as a left outer join " \
...
if log_time < '0100':
        ...
        sql = "(select distinct a.* from (select * from file2dw_log where game='%s' and platform='%s' " \
              "and load_status=0 and do_rate='%s' and task_date='%s' and in_queue=0 and exec_num<4 and log_time<='%s') " \
              ...
              "and a.platform=b.platform and a.log_name=b.log_name and a.task_date = b.task_date " \
              "and a.do_rate=b.do_rate and a.log_time=b.log_time) union (select distinct a.* from " \
              "(select * from file2dw_log where game='%s' and platform='%s' and load_status=0 and do_rate='%s' " \
              "and task_date='%s' and in_queue=0 and exec_num<4 and log_time<='%s') as a left outer join " \
              "(select * from etl_data_log where game='%s' and platform='%s' and etl_status=6 and download_status=3 " \
              "and do_rate='%s' and task_date='%s' and log_time<='%s') as b on a.game=b.game " \
              "and a.platform=b.platform and a.log_name=b.log_name and a.task_date = b.task_date " \
        ...
        if result is not None:
            ...
            if in_queue == 0:
                if exec_num < 3:
                    ...
                    if update_result == 1:
                        run_task.apply_async((result,), queue='file2dw')
                    else:

...
__ALL__=['AddDm2ReportTask']
class AddDm2ReportTask(BaseAddReportTask):
    ...
    def make_task_log(self,tasks,task_log,log_date,task_date):
        ...
        for task in tasks: 
            ...
            log_info = [task['game'],task['platform'],task['task_name'],task['date_cycle'],task['do_rate'],
                        task['group'],task['priority'],task['replace_prefix_sql'],task['replace_exec_sql'],task['post_sql'],
                        task['from_id'],task['target_id'],log_date,task_date,task['comment']]
    ...
    def save_task(self,param):
        '''
            存储
        '''
        sql = 'INSERT INTO dm2report_log \
        (`game`,`platform`,`task_name`,`date_cycle`,`do_rate`,`group`,`priority`,`prefix_sql`,`exec_sql`,`post_sql`,`from_id`,`target_id`,`log_date`,`task_date`,`comment`) \
    ...
    def run(self,game = None,platform = "all",task_name = None,cycle = None):
        ...
        result=self.get_task('select a.*,s.db_name,s.table_name from dm2report a left join structure s on a.target_id = s.id where  a.is_delete = 0 %s' % condition)
...
if __name__ == '__main__':
    ...
    task = AddDm2ReportTask(options.start_date,options.end_date,options.platform,options.do_rate,options.time_zone)

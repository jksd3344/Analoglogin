...
class AddEtlDataTask(BaseAddTask,CenterApp):
    ...
    def make_task_log(self,tasks,log_date,log_time,do_rate='hour'):
        ...
        for task in tasks:
            ...
            for ip in task_ip:
                ...
                #print log_info
                output = open('/data/add_data/etl_data_'+str(time.strftime("%Y%m%d",time.localtime()))+'.txt', 'a')
    ...
    def save_task(self,logtime):
        result = self.mysql.load("load data local infile '/data/add_data/etl_data_"+logtime+".txt' into table etl_manage.etl_data_log(game,platform,source_ip,log_name,log_dir,md5_name,md5_dir,download_url,col_num,do_rate,\`group\`,priority,from_id,target_id,log_date,log_time,task_date);")
        return result
...
if __name__ == '__main__':
    ...
    if os.path.exists('/data/add_data/etl_data_'+logtime+'.txt'):
        cmd.run('rm -f /data/add_data/etl_data_'+logtime+'.txt')

    ip_list = task.get_ip_list()
    task.get_task('select etl.*,s.column_name,s.flag from etl_data etl left join structure s on etl.target_id = s.id where etl.type = "download" and etl.is_delete = 0',ip_list)

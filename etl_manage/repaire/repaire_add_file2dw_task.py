...
if __name__ == '__main__':
    ...
    task = AddFile2DwTask(options.start_time,options.end_time)
    ...
    task.get_task('select etl.*,s.column_name,s.flag from etl_data etl left join structure s on etl.target_id = s.id where %s and etl.is_delete = 0 and etl.type = "load" and etl.db_type = "hive"' % condition,ip_list)

...
class CopyConfig():
    ...
    def get_all_task(self,task_name):
        ...
        ##appstoremix
        task_list = self.mysql.query("select * from dm2report_new where is_delete = 0 and platform='all' and %s" % (condition))
        return task_list
    ...
    def save_new_task(self,task):
        self.mysql.insert("dm2report_new",**task)
        self.mysql.commit()
...
if __name__ == '__main__':
    ...
    parser.add_option(
        '--platform',
        dest='platform',
        help='plat_form add stucture platform can use.',
        metavar='PLATFORM',
    )
